...
GitHub &
\url{https://github.com/docxology/template_template/releases/tag/v1.0.9} \\
Zenodo & \url{https://zenodo.org/records/20419007} \\
SHA-256 & \breaktt{535bd80943d0ae9f…} \\
SHA-512 & pending \\
\end{longtable}
}
...
packages, about 616 modules, validated by about 7,968
tests) from self-contained project workspaces, connected by a
YAML-declared pipeline (14 stages; default full 10)-based build pipeline
...
(\breaktt{template\_code\_project}, 236 tests), prose
(\breaktt{template\_prose\_project}, 120 tests), and AutoResearch
readiness (\breaktt{template\_autoresearch\_project}, 296 tests). These
...
(130 tests) as a public exemplar in the same discovered/rendered tree,
...
\item
  ...
  packages) comprising about 616 Python modules provide
...
\item
  ...
  test doubles. about 7,968 infrastructure tests and 3390+
...
\item
  A detailed specification of the 14-stage DAG in
  \breaktt{infrastructure/core/pipeline/pipeline.yaml}: default full runs
  use 10 stages; \texttt{-\/-core-only} runs 8; opt-in ebook, metadata,
  bundle, and archival stages via \texttt{-\/-tags}.
\item
...
Python packages---comprising about 616 modules and providing
...
\item
  \textbf{PDF Rendering} (\breaktt{03\_render\_pdf.py}) --- Pandoc \texorpdfstring{\ensuremath{\to}}{->}
  XeLaTeX loop, bibliography assembly, injected variables from Stage 02
  artefacts.
\item
...
\textbf{Stage 02 illustration.} The analysis stage is deliberately
concrete rather than a hypothetical diagram factory: each canonical
project ships real behaviour at this node.
\breaktt{template\_autoresearch\_project} runs readiness validation;
\breaktt{template\_search\_project} merges remote literature JSON,
generates scripted figures (\breaktt{y\_generate\_search\_figures.py}),
and writes manifests; \breaktt{template\_code\_project} emits
optimization plots; and \breaktt{template\_prose\_project} triggers
structural validation scaffolding. The pipeline shape is identical
across all four---only the Stage 02 payload differs---which is exactly
...
\texttt{template/} skill architecture maps cleanly onto these three
capabilities: the \texttt{SKILL.md} descriptors supply the tool-use
layer, the declarative DAG of \texttt{14} \texttt{pipeline.yaml} stages
(a default full run executes \texttt{10}) supplies the planning
scaffold, and the per-criterion checkpoint system supplies the memory
layer.
...
\tightlist
\item
  \textbf{Infrastructure tests} (\texttt{tests/}): about 7,968
...
\bottomrule\noalign{}
\endlastfoot
\breaktt{template\_code\_project} & 8 & about 40 s & 236/236 &
90\%+ \\
\breaktt{template\_prose\_project} & 8 & about 35 s & 120/120 &
90\%+ \\
...
\bottomrule\noalign{}
\endlastfoot
\texttt{autoresearch} & 10 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \breaktt{build\_autoresearch\_plan},
...
templates \\
\texttt{core} & 110 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texttt{get\_logger},
...
management \\
\texttt{prose} & 9 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & Markdown readability + prose tooling \\
\texttt{publishing} & 74 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & Zenodo, executable bundle, archival
targets \\
...
\texttt{rendering} & 57 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & PDF/HTML/slide rendering, Pandoc
filters \\
...
clients + cache \\
\texttt{sia} & 10 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & Self-Improving-AI loop: task validation,
harness, metric capture \\
\texttt{skills} & 7 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texttt{discover\_skills}, SKILL manifest
regeneration \\
...
manifests \\
\texttt{validation} & 84 & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & \texorpdfstring{\ensuremath{\checkmark}}{[ok]} & PDF + Markdown + integrity CLIs \\
\end{longtable}
}
...
\texttt{380+} Markdown shards under \texttt{docs/} capture operational
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Two-Layer Architecture Overview}]{../figures/architecture_overview.png}}
...
\texttt{\texorpdfstring{\ensuremath{\cdot}}{.}} marking an absent file---so a fully documented module reads
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Pipeline Stage Flow}]{../figures/pipeline_stages.png}}
\textbf{Figure 2.} Pipeline DAG with 14 YAML-declared stages (core, LLM,
ebook, metadata, bundle, archival tags).

\pandocbounded{\includegraphics[keepaspectratio,alt={Infrastructure Module Inventory}]{../figures/module_inventory.png}}
...
\pandocbounded{\includegraphics[keepaspectratio,alt={Comparative Feature Analysis Heatmap}]{../figures/comparative_feature_matrix.png}}
...
duration: the full infrastructure test suite (about 7,968
...
We have validated scaling with 16 canonical exemplars under
\breaktt{projects/templates/}---always present for onboarding and
tooling---and with this manuscript from
\breaktt{projects/templates/template\_template} (130 tests) as a
...
\item
  ...
  example with gradient-descent narration, 236 tests, 90\%+ coverage.
  Minimal footprint: compact \texttt{src/}, scripted analysis, short
  manuscript sections.
\item
  \textbf{\breaktt{template\_prose\_project}}: Prose-heavy manuscript
  emphasizing narrative structure and bibliography discipline, 120
  tests, 90\%+ coverage---tests exercise rendering and Markdown
  integrity without heavyweight numerics.
\item
  \textbf{\breaktt{template\_autoresearch\_project}}: AutoResearch
  readiness workflow invoking
  \breaktt{projects/templates/template\_search\_project/scripts/} to run
  corpus builders, scripted figures (\texttt{../figures/}), and
  manifold-variable injection (the literature-search exemplar). Typical
  Stage~02 workloads include bibliography fusion, corpus JSON assembly,
  deep-search aggregates, and report composition.
\end{itemize}
...
infrastructure subdirectories, about 616 Python
...
project size: the same 23 modules, the same 13 pipeline stages, the same
...
software engineering literature. The scale of the investment is
substantial: \texttt{380} Markdown files under \texttt{docs/} alone,
plus an \texttt{AGENTS.md}/\texttt{README.md} pair in every directory
and a \texttt{SKILL.md} descriptor on every infrastructure module. That
count is itself injected from live introspection---the manuscript
refuses to quote a documentation total it cannot recompute---and it
represents a deliberate commitment to machine-readable context that
shrinks the surface on which an agent can hallucinate.
...
totals use \texttt{616} Python sources across infra + \texttt{7,968}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.autoresearch} (10
files)}{infrastructure.autoresearch (10 files)}}\label{infrastructure.autoresearch-10-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.core} (110
files)}{infrastructure.core (110 files)}}\label{infrastructure.core-110-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.prose} (9
files)}{infrastructure.prose (9 files)}}\label{infrastructure.prose-9-files}

Readability metrics + Markdown tooling for prose-centric manuscripts /
CI gates.

\subsubsection{\texorpdfstring{\breaktt{infrastructure.publishing} (74
files)}{infrastructure.publishing (74 files)}}\label{infrastructure.publishing-74-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.rendering} (57
files)}{infrastructure.rendering (57 files)}}\label{infrastructure.rendering-57-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.sia} (10
files)}{infrastructure.sia (10 files)}}\label{infrastructure.sia-10-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.skills} (7
files)}{infrastructure.skills (7 files)}}\label{infrastructure.skills-7-files}
...
\subsubsection{\texorpdfstring{\breaktt{infrastructure.validation} (84
files)}{infrastructure.validation (84 files)}}\label{infrastructure.validation-84-files}
...
figures, scripted tables & 236 @ 90\%+ gate & Controlled matplotlib
exports \\
\breaktt{template\_prose\_project} & Prose-heavy workflow & Validates
narrative-only repos & 120 & Lightweight / optional plots \\
\breaktt{template\_autoresearch\_project} & AutoResearch readiness &
Planner + validation CLI & 296 & Readiness reports from Stage 02 \\
\end{longtable}
}
...
agent context \\
\texttt{docs/} directory & 380 files across 17 subdirectories & User
...
(21 AI prompt templates), \texttt{security/} (steganography, hashing),
and \texttt{audit/} (review reports).

Every count in this appendix is injected from live repository
introspection rather than hand-maintained: \texttt{380} counts every
Markdown file beneath \texttt{docs/} recursively, \texttt{17} counts its
first-level subdirectories, and \texttt{21} counts the workflow
subdirectories that each carry a \texttt{SKILL.md} descriptor. This is
the same discipline the manuscript argues for throughout---a hand-typed
``90+ files across 12 subdirectories'' silently rots as the tree grows,
whereas a token re-resolves on every render. A reader onboarding to the
repository should start at \texttt{docs/core/}, follow the graduated
\texttt{docs/guides/} skill ladder, and consult the per-directory
\texttt{AGENTS.md}/\texttt{README.md} pair nearest to whatever code they
are editing; AI agents additionally read each module's \texttt{SKILL.md}
to locate capabilities without guessing API signatures.
...
\textbf{Release:} v1.0.9 \texorpdfstring{\ensuremath{\cdot}}{.} DOI \breaktt{10.5281/zenodo.20419007} \texorpdfstring{\ensuremath{\cdot}}{.}
SHA-256 \texttt{535bd80943d0…} \texorpdfstring{\ensuremath{\cdot}}{.} pairing complete
...
\texttt{cc674248…} \texorpdfstring{\ensuremath{\cdot}}{.} \texttt{v1.0.8} \texorpdfstring{\ensuremath{\cdot}}{.} \breaktt{10.5281/zenodo.20932076}
\texorpdfstring{\ensuremath{\cdot}}{.} \texttt{b9bc5cf3…}
